...
class WorldEditorFoundationTest(unittest.TestCase):
    ...
    def test_layered_builder_allocation_is_explicit_live_and_sparse(self):
        ...
        self.assertIn("Save and reopen the Builder", commands)
        provisioned = commands.index("provisionNativeNavigationTarget(player,x,y,level)")
        deferred = commands.index("return;", provisioned)
        teleported = commands.index("player.teleportLayered(destination,false)", provisioned)
        self.assertLess(deferred, teleported)
    ...
    def test_scenery_and_npc_tools_delegate_to_established_commands(self):
        ...
        self.assertEqual(6, sessions.count("requireClientPlacementDefinition("))
        ...
        self.assertIn("player.getClientLimitations().maxBoundaryId", sessions)
        self.assertIn("placeNativeNpc(player,id,radius,respawnSeconds,x,y)", commands)
        self.assertIn("hasPublishedNativeNavigationTerrain", commands)
        self.assertIn("hasUnpublishedNativeNavigationTerrain", commands)
        self.assertIn("Save and reopen the Builder before navigating there", commands)
    ...
    def test_terrain_t1_is_bounded_server_authoritative_and_durably_saveable(self):
        ...
        self.assertIn("rectangleOptions.isEastWall()?16:0", ui)
        self.assertIn("rectangleOptions.isNorthWall()?32:0", ui)
        self.assertIn("rectangleOptions.isDiagonalWall()?64:0", ui)
        ...
        self.assertIn("WorldEditorTerrainBrush.centeredFootprint", ui)
        self.assertNotIn("centeredThreeByThree", ui)
        self.assertIn("terrainBrushSize=5", ui)
        self.assertIn("terrainBrushSize=7", ui)
        self.assertIn('button(x+270,y+194,105,"Save edits")', ui)
        self.assertIn('button(x+270,y+248,105,"Save edits")', ui)
