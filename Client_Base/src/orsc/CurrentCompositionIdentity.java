...
/** Runtime authority for a provider-bound current desktop client composition. */
public final class CurrentCompositionIdentity {
	...
	public static synchronized CurrentCompositionIdentity initializeFromSystemProperties() {
		...
		if (marker == null) {
			if (!configured.isEmpty()) {
				throw new IllegalArgumentException(
					"composition identity was supplied to an unbound client artifact");
			}
			current = disabled();
			return current;
		}
		if (configured.isEmpty()) {
			closeQuietly(marker);
			throw new IllegalArgumentException(
				IDENTITY_FILE_PROPERTY + " is required by this client artifact");
		}
		try {
			...
			throw new IllegalArgumentException(
				"cannot load current client composition identity: "
	...
	public void writeHandshake(RSBuffer_Bits buffer) {
		if (!enabled) {
			throw new IllegalArgumentException("client composition identity is disabled");
		}
		buffer.putString(HANDSHAKE_ID);
		for (String field : IDENTITY_FIELDS) {
			buffer.putString(value(field));
		}
	}
	...
	private void requireArtifactMarker(InputStream input) throws IOException {
		...
		if (!marker.stringPropertyNames().equals(MARKER_KEYS)) {
			throw new IOException("client artifact marker keys differ from the contract");
		}
		if (!ARTIFACT_CONTRACT.equals(marker.getProperty("artifactContract"))
			|| !HANDSHAKE_ID.equals(marker.getProperty("handshakeId"))) {
			throw new IOException("client artifact marker has wrong contract identity");
		}
		for (String field : IDENTITY_FIELDS) {
			...
			if (!value(field).equals(marker.getProperty(field))) {
				throw new IOException("client artifact differs at " + field);
			}
		}
	}
	...
	public static void main(String[] args) {
		try {
			...
			if (!identity.isEnabled()) {
				throw new IllegalArgumentException("client artifact is not composition-bound");
			}
			System.out.println("Current client composition accepted: "
				+ identity.value("variantId"));
		} catch (IllegalArgumentException exception) {
			System.err.println("Current client startup refused: " + exception.getMessage());
			System.exit(2);
		}
	}
}
