...
public final class WorldEditorInterface extends NCustomComponent {
	...
	private static final int DOCK_WIDTH=70,DOCK_HEIGHT=336,FLYOUT_WIDTH=180,FLYOUT_GAP=4,EXPANDED_WIDTH=455;
	...
	public enum RegionTool { COPY, PASTE }
	private static final String[] TABS={"Navigate","Inspect","Terrain","Scenery","NPC","Items","Copy/Paste"};
	...
	private boolean regionPasteOverwriteArmed=false;
	...
	public void open(long id,int sequence){
		...
		sessionId=id;nextSequence=sequence;mode=Mode.NAVIGATE;terrainTool=TerrainTool.FREEHAND;clearTerrainLine();clearSceneryMove();clearRegionSelection();resetRegionPaste();regionCopyBridge.reset();regionPasteBridge.reset();toolbar.reset();definitionBrowser.close();icons.initialize();
	...
	public boolean isRegionPasteSelectingDestination(){return isEditorOpen()&&mode==Mode.REGION&&regionTool==RegionTool.PASTE&&!regionPasteBridge.isPending()&&!regionLibrary.isEmpty();}
	...
	private void removeLastRegionMarker(){
		...
		int[] removed=regionMarkers.remove(regionMarkers.size()-1);regionClosed=false;regionPreviewTiles=new int[0][2];regionHoverX=removed[0];regionHoverY=removed[1];inspectionStatus="Removed the last region marker; "+regionMarkers.size()+" remain.";
	}
	private void closeRegionSelection(){
		...
		try{int[][] markers=regionMarkerTiles();WorldEditorRegionSelection.validateClosed(markers);regionPreviewTiles=WorldEditorRegionSelection.ownedTiles(markers,65536);regionClosed=true;inspectionStatus="Region closed: "+regionPreviewTiles.length+" terrain tiles. Review the preview, name it, then Copy.";}
	...
	private void reopenRegionSelection(){if(regionCopyBridge.isPending())return;regionClosed=false;regionPreviewTiles=new int[0][2];inspectionStatus="Region reopened; add a marker or remove the last marker before closing again.";}
	...
	private void cancelRegionSelection(){if(regionCopyBridge.isPending()){inspectionStatus="Region Copy is already running; wait for its response.";return;}clearRegionSelection();inspectionStatus="Region selection cancelled; the world and snapshot library were unchanged.";}
	private void requestRegionCopy(){
		if(!regionClosed){inspectionStatus="Close a valid region selection before copying it.";return;}
		if(terrainStrokeTiles!=null||terrainLineCommitTiles!=null||terrainDragActive||terrainDragReleasePending||pendingEntityActions>0){inspectionStatus="Wait for authoritative edit responses before copying the region.";return;}
	...
	private void pollRegionCopy(){
		...
		if(result.accepted){saveRequested=false;unsavedChanges=false;lastRegionSnapshotId=result.snapshotId;lastRegionSnapshotName=result.name;lastRegionTileCount=result.tileCount;lastRegionPlacementCount=result.placementCount;lastRegionCrossingCount=result.crossingReportCount;inspectionStatus="Copied '"+result.name+"': "+result.tileCount+" tiles, "+result.placementCount+" placements, snapshot "+shortHash(result.snapshotId)+(result.crossingReportCount>0?"; "+result.crossingReportCount+" crossing footprint report(s).":".");if(regionTool==RegionTool.PASTE)requestRegionLibrary();}
	...
	private void selectRegionTool(RegionTool selected){
		if(regionCopyBridge.isPending()||regionPasteBridge.isPending()){inspectionStatus="Wait for the active region operation before changing Copy/Paste tools.";return;}
	...
	private void requestRegionLibrary(){
		if(regionPasteBridge.isPending())return;
	...
	private void cycleRegionSnapshot(int amount){
		if(regionPasteBridge.isPending()||regionLibrary.isEmpty())return;
	...
	public void setRegionPasteDestination(int worldX,int worldY){
		if(!isRegionPasteSelectingDestination())return;WorldBuilderRegionPasteClientBridge.Snapshot snapshot=selectedRegionSnapshot();if(snapshot==null)return;
	...
	private void requestRegionPasteApply(){
		if(regionPasteBridge.isPending())return;if(regionPastePlanHash.isEmpty()){inspectionStatus="Choose a destination and wait for its exact Paste preview first.";return;}if(regionPasteBlocked){inspectionStatus="This Paste plan is blocked; choose another destination or compatible snapshot.";return;}
	...
	private void pollRegionPaste(){
		...
		if("library".equals(result.operation)){String selected=selectedRegionSnapshot()==null?"":selectedRegionSnapshot().id;regionLibrary.clear();regionLibrary.addAll(result.snapshots);regionLibraryIndex=0;for(int i=0;i<regionLibrary.size();i++)if(regionLibrary.get(i).id.equals(selected)){regionLibraryIndex=i;break;}clearPastePreview();inspectionStatus=regionLibrary.isEmpty()?"The project snapshot library is empty. Use Copy first or import a .wbr snapshot.":"Loaded "+regionLibrary.size()+" verified snapshot"+(regionLibrary.size()==1?"":"s")+". Select one, then click terrain to place marker 1.";return;}
		...
		if("apply".equals(result.operation)){saveRequested=false;unsavedChanges=false;inspectionStatus="Region pasted atomically. Activating the published region live...";mc.sendCommandString("activateregionpaste "+result.requestId);}
	}
	private void clearPastePreview(){regionPastePreviewTiles=new int[0][2];regionPasteCollisionTiles=new int[0][2];regionPastePlanHash="";regionPasteBlocked=false;regionPasteOverwrite=false;regionPasteOverwriteArmed=false;}
	private void resetRegionPaste(){regionTool=RegionTool.COPY;regionLibrary.clear();regionLibraryIndex=-1;regionPasteX=regionPasteY=-1;regionPasteLevel=0;clearPastePreview();}
	...
	public void observeGameMessage(String message){
		...
		if(message.contains("Region Paste activated live")){
			clearPastePreview();inspectionStatus="Region pasted and activated live; the Builder stayed open.";
		}else if(message.contains("Live Region Paste activation failed:"))inspectionStatus=message;
	}
	...
	public void acceptTerrainLineChunk(int sequence,int fieldMask,int total,int offset,int[][] tiles,boolean[] projectiles,String definitions){
		...
		long responseNanos=System.nanoTime();terrainLineLastResponseNanos=responseNanos;if(offset==0){int[] center=tiles[0];
			showTerrain(sequence,center[0],center[1],center[2],center[3],center[4],center[5],center[6],center[7],center[8],center[9],center[10],center[11],center[12],center[13],center[14],projectiles[0],false,definitions);}
	...
	private void commitTerrainLine(int worldX,int worldY,int mask){
		...
		int startX=terrainLineAnchorX,startY=terrainLineAnchorY;clearTerrainDrag();terrainGestureLabel="Line";snapshotTerrainPaint(mask);
	...
	private void commitTerrainRectangle(int worldX,int worldY,int mask){
		...
		int startX=terrainLineAnchorX,startY=terrainLineAnchorY;clearTerrainDrag();terrainGestureLabel="Rectangle";snapshotTerrainPaint(mask);
	...
	public boolean updateTerrainDrag(boolean controlDown,boolean primaryDown,int worldX,int worldY){
		...
		if(!terrainDragActive){
			...
			clearTerrainDrag();terrainGestureLabel="Brush";terrainDragActive=true;snapshotTerrainPaint(mask);addTerrainDragCenter(worldX,worldY,now);inspectionStatus=terrainDragStatus();return true;
		}
	...
	private void sendTerrainLine(int startX,int startY,int endX,int endY){
		...
		mc.packetHandler.getClientStream().bufferBits.putByte(terrainStrokeTexture);mc.packetHandler.getClientStream().bufferBits.putByte(terrainStrokeRoof);
		mc.packetHandler.getClientStream().bufferBits.putByte(terrainStrokeEastWall);mc.packetHandler.getClientStream().bufferBits.putByte(terrainStrokeNorthWall);
		mc.packetHandler.getClientStream().bufferBits.putInt(terrainStrokeDiagonal);mc.packetHandler.getClientStream().bufferBits.putByte(terrainBrushSize);
	...
	private void sendTerrainRectangle(int startX,int startY,int endX,int endY){
		...
		mc.packetHandler.getClientStream().bufferBits.putByte(terrainStrokeTexture);mc.packetHandler.getClientStream().bufferBits.putByte(terrainStrokeRoof);
		mc.packetHandler.getClientStream().bufferBits.putByte(terrainStrokeEastWall);mc.packetHandler.getClientStream().bufferBits.putByte(terrainStrokeNorthWall);
		mc.packetHandler.getClientStream().bufferBits.putInt(terrainStrokeDiagonal);
	...
	private boolean handleMouse(int mx,int my,int down,int click){
		...
		if(rx<DOCK_WIDTH){
			...
			if(dockHit(rx,ry,0,9)){if(click==1)requestWorldEditSave();return true;}
			if(dockHit(rx,ry,0,10)){if(click==1)requestEditorClose();return true;}
			return true;
		}
	...
	private int contextActionAtDock(int x,int y){
		...
		else if(mode==Mode.REGION){if(dockHit(x,y,1,0))return 0;if(dockHit(x,y,1,1))return 1;if(dockHit(x,y,1,2))return 2;if(dockHit(x,y,1,3))return 3;if(dockHit(x,y,1,4))return 4;if(dockHit(x,y,1,5))return 5;}
		return -1;
	}
	private void selectContextAction(int action){
		...
		else if(mode==Mode.REGION){if(action==0)selectRegionTool(RegionTool.COPY);else if(action==1)selectRegionTool(RegionTool.PASTE);else if(regionTool==RegionTool.COPY){if(action==2){if(regionClosed)reopenRegionSelection();else closeRegionSelection();}else if(action==3)removeLastRegionMarker();else if(action==4)requestRegionCopy();else cancelRegionSelection();}else{if(action==2)cycleRegionSnapshot(-1);else if(action==3)cycleRegionSnapshot(1);else if(action==4)requestRegionLibrary();else requestRegionPasteApply();}}
		closeArmed=false;
	}
	...
	private void handleCompactTerrainMouse(int x,int y){
		...
		int toggleY=terrainActiveField==6&&isLayeredTerrainDraft()?150:terrainActiveField==8?128:112;if(y>=toggleY&&y<toggleY+24){toggleTerrainField(terrainActiveField);return;}
	...
	private void handleCompactRegionMouse(int x,int y){
		if(y>=44&&y<68){selectRegionTool(x<88?RegionTool.COPY:RegionTool.PASTE);return;}
		if(regionTool==RegionTool.COPY){if(y>=76&&y<100){coordinateFocus=19;replaceFocusedText=true;return;}if(y>=108&&y<132){if(x<88){if(regionClosed)reopenRegionSelection();else closeRegionSelection();}else removeLastRegionMarker();return;}if(y>=140&&y<164){if(x<88)requestRegionCopy();else cancelRegionSelection();}}
		else{if(y>=92&&y<116){if(x<88)cycleRegionSnapshot(-1);else cycleRegionSnapshot(1);return;}if(y>=140&&y<164){if(x<88)requestRegionLibrary();else requestRegionPasteApply();}}
	}
		...
		if(click==1){
			...
			if(mode==Mode.TERRAIN){
				...
				if(!terrainStructureTab){
					...
					if(terrainTool==TerrainTool.RECTANGLE&&rectangleOptions.isSmartWalls()){if(ry>=118&&ry<142){rectangleOptions.setSmartWalls(false);clearTerrainLine();return true;}if(ry>=145&&ry<169){if(rx<105)rectangleOptions.toggleNorthWall();else if(rx<205)rectangleOptions.toggleEastWall();return true;}if(ry>=170&&ry<194){if(rx>=118&&rx<142)setTerrainSmartWall(steppedWallValue(terrainSmartWall,-1));else if(rx>=148&&rx<202)focusNumber(18);else if(rx>=208&&rx<232)setTerrainSmartWall(steppedWallValue(terrainSmartWall,1));return true;}if(ry>=194&&ry<218){if(rx>=65&&rx<137)rectangleOptions.setFill(false);else if(rx>=141&&rx<196)rectangleOptions.setFill(true);else if(rx>=270&&rx<375)requestWorldEditSave();clearTerrainLine();return true;}return true;}
					if(ry>=118&&ry<142){if(rx>=10&&rx<30)rectangleOptions.toggleNorthWall();else if(rx>=118&&rx<142)setTerrainNorthWall(steppedWallValue(terrainNorthWall,-1));else if(rx>=148&&rx<202)focusNumber(10);else if(rx>=208&&rx<232)setTerrainNorthWall(steppedWallValue(terrainNorthWall,1));return true;}
					if(ry>=154&&ry<178){if(rx>=10&&rx<30)rectangleOptions.toggleEastWall();else if(rx>=118&&rx<142)setTerrainEastWall(steppedWallValue(terrainEastWall,-1));else if(rx>=148&&rx<202)focusNumber(11);else if(rx>=208&&rx<232)setTerrainEastWall(steppedWallValue(terrainEastWall,1));return true;}
					if(ry>=190&&ry<214){if(rx>=10&&rx<30)rectangleOptions.toggleDiagonalWall();else if(rx>=118&&rx<142)setTerrainDiagonalWall(steppedWallValue(terrainDiagonalWall,-1));else if(rx>=148&&rx<202)focusNumber(12);else if(rx>=208&&rx<232)setTerrainDiagonalWall(steppedWallValue(terrainDiagonalWall,1));return true;}
			...
			if(mode==Mode.REGION){
				if(ry>=56&&ry<80){if(rx>=10&&rx<120)selectRegionTool(RegionTool.COPY);else if(rx>=128&&rx<238)selectRegionTool(RegionTool.PASTE);return true;}
				if(regionTool==RegionTool.COPY){if(ry>=104&&ry<128&&rx>=118&&rx<378){coordinateFocus=19;replaceFocusedText=true;return true;}if(ry>=145&&ry<169){if(rx>=10&&rx<110){if(regionClosed)reopenRegionSelection();else closeRegionSelection();}else if(rx>=118&&rx<218)removeLastRegionMarker();else if(rx>=226&&rx<326)cancelRegionSelection();return true;}if(ry>=188&&ry<212&&rx>=10&&rx<175){requestRegionCopy();return true;}}
				else{if(ry>=112&&ry<136){if(rx>=10&&rx<110)cycleRegionSnapshot(-1);else if(rx>=118&&rx<218)cycleRegionSnapshot(1);else if(rx>=226&&rx<326)requestRegionLibrary();return true;}if(ry>=188&&ry<212&&rx>=10&&rx<175){requestRegionPasteApply();return true;}}
			}
			coordinateFocus=0;
		}
	...
	private void renderCompact(){
		...
		drawIconButton(WorldEditorIconRegistry.Key.ACTION_SAVE,x+DOCK_LEFT,y+dockRowY(9),false,false,false,unsavedChanges||saveRequested);
		drawIconButton(WorldEditorIconRegistry.Key.ACTION_CLOSE,x+DOCK_LEFT,y+dockRowY(10),false,false,closeArmed,false);
	...
	private void renderDockContextActions(int x,int y){
		if(mode==Mode.TERRAIN){
			...
			drawContextActionIcon(WorldEditorIconRegistry.Key.MODE_REGION,"Copy",x+DOCK_RIGHT,y+dockRowY(0),regionTool==RegionTool.COPY);
			drawContextActionIcon(WorldEditorIconRegistry.Key.MODE_REGION,"Paste",x+DOCK_RIGHT,y+dockRowY(1),regionTool==RegionTool.PASTE);
			if(regionTool==RegionTool.COPY){drawContextActionIcon(WorldEditorIconRegistry.Key.ACTION_CLOSE,regionClosed?"Open":"Close",x+DOCK_RIGHT,y+dockRowY(2),regionClosed);drawContextActionIcon(WorldEditorIconRegistry.Key.ACTION_PIN,"Undo",x+DOCK_RIGHT,y+dockRowY(3),false);drawContextActionIcon(WorldEditorIconRegistry.Key.MODE_REGION,"Copy",x+DOCK_RIGHT,y+dockRowY(4),regionCopyBridge.isPending());drawContextActionIcon(WorldEditorIconRegistry.Key.ACTION_CLOSE,"Clear",x+DOCK_RIGHT,y+dockRowY(5),false);}
			else{drawContextActionIcon(WorldEditorIconRegistry.Key.ACTION_PIN,"Prev",x+DOCK_RIGHT,y+dockRowY(2),false);drawContextActionIcon(WorldEditorIconRegistry.Key.ACTION_PIN,"Next",x+DOCK_RIGHT,y+dockRowY(3),false);drawContextActionIcon(WorldEditorIconRegistry.Key.MODE_REGION,"Load",x+DOCK_RIGHT,y+dockRowY(4),regionPasteBridge.isPending());drawContextActionIcon(WorldEditorIconRegistry.Key.ACTION_SAVE,regionPasteOverwrite?"Over":"Apply",x+DOCK_RIGHT,y+dockRowY(5),regionPasteOverwriteArmed);}
		}
	}
	...
	private void renderCompactTerrain(int x,int y){
		...
		int toggleY=112;if(terrainActiveField==6&&isLayeredTerrainDraft()){toolButton(x+8,y+90,50,"Set",terrainElevationOperation==0);toolButton(x+60,y+90,52,"Raise",terrainElevationOperation==1);toolButton(x+114,y+90,54,"Lower",terrainElevationOperation==2);button(x+8,y+120,30,"-");textField(x+42,y+120,88,terrainElevationStepText,coordinateFocus==17);button(x+134,y+120,30,"+");toggleY=150;}else if(terrainActiveField==8){graphics().drawString(compactLine(floorTextureVisualName(),28),x+8,y+99,terrainFieldInvalid(terrainActiveField)?0xff981f:0xffffff,1);
	...
	private void renderCompactNpc(int x,int y){
		...
		button(x+8,y+138,164,"Browse NPCs...");
	}
	...
	private void renderCompactRegion(int x,int y){
		...
		else{WorldBuilderRegionPasteClientBridge.Snapshot snapshot=selectedRegionSnapshot();graphics().drawString(compactLine(snapshot==null?"No snapshots":snapshot.name,28),x+8,y+83,snapshot==null?0xff981f:0xffff00,1);button(x+8,y+92,78,"Previous");button(x+94,y+92,78,"Next");graphics().drawString(regionPasteX<0?"Click terrain for marker 1.":"Anchor: "+regionPasteX+","+regionPasteY+",L"+regionPasteLevel,x+8,y+132,0xffffff,1);button(x+8,y+140,78,regionPasteBridge.isPending()?"Working...":"Refresh");button(x+94,y+140,78,regionPasteBlocked?"Blocked":regionPasteOverwrite?(regionPasteOverwriteArmed?"Confirm":"Overwrite"):"Apply");graphics().drawString(regionPastePlanHash.isEmpty()?"Preview appears after destination click.":regionPastePreviewTiles.length+" tiles | "+regionPasteCollisionTiles.length+" collision markers",x+8,y+179,regionPasteBlocked?0xff4040:regionPasteOverwrite?0xff981f:0x80c080,1);}
	}
	...
	private String toolbarTooltipAt(int x,int y){
		...
		if(dockHit(x,y,0,9))return "Save | "+(unsavedChanges?"unsaved changes":"clean")+(saveRequested?" | requested":"");if(dockHit(x,y,0,10))return closeArmed?"Close without saving: confirm":"Close editor";return "";
	}
	private String contextActionTooltip(int action){
		...
		if(action==0)return "Region Copy tool";if(action==1)return "Region Paste tool";if(regionTool==RegionTool.COPY)return "Region Copy: "+(action==2?(regionClosed?"Reopen":"Close"):action==3?"Undo last marker":action==4?"Copy to library":"Clear selection");return "Region Paste: "+(action==2?"Previous snapshot":action==3?"Next snapshot":action==4?"Refresh library":regionPasteOverwrite?"Apply with overwrite confirmation":"Apply exact preview");
	}
	...
	private void renderRegion(int x,int y){
		...
		else{WorldBuilderRegionPasteClientBridge.Snapshot snapshot=selectedRegionSnapshot();graphics().drawString(snapshot==null?"No verified snapshots are in this project library.":"Selected: "+snapshot.name+" ["+shortHash(snapshot.id)+"]",x+10,y+101,snapshot==null?0xff981f:0xffff00,2);button(x+10,y+112,100,"Previous");button(x+118,y+112,100,"Next");button(x+226,y+112,100,regionPasteBridge.isPending()?"Working...":"Refresh library");if(snapshot!=null)graphics().drawString(snapshot.tileCount+" tiles, "+snapshot.placementCount+" placements, "+snapshot.levelCount+" level(s)",x+10,y+160,0xbdbdbd,2);graphics().drawString(regionPasteX<0?"Click a destination tile; marker 1 will anchor there.":"Destination marker 1: "+regionPasteX+", "+regionPasteY+", level "+regionPasteLevel,x+10,y+180,0xffffff,2);button(x+10,y+188,165,regionPasteBridge.isPending()?"Working...":regionPasteBlocked?"Paste blocked":regionPasteOverwrite?(regionPasteOverwriteArmed?"Confirm overwrite":"Apply overwrite"):"Apply Paste");graphics().drawString(regionPastePlanHash.isEmpty()?"A visible exact ghost appears after the destination preview.":regionPastePreviewTiles.length+" preview tiles | "+regionPasteCollisionTiles.length+" visible collision marker(s)",x+190,y+205,regionPasteBlocked?0xff4040:regionPasteOverwrite?0xff981f:0x80c080,2);graphics().drawString(regionPasteOverwrite?"Overwrite requires a second explicit click bound to this exact plan.":"Paste is atomic and activates live without restarting the Builder.",x+10,y+235,0xff981f,1);graphics().drawString(inspectionStatus,x+10,y+264,0xbdbdbd,1);}
	}
