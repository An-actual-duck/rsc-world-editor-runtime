...
The v0.8.0 release check also admits the exact fixed `SEALED_V080` tuple in
`preservation_semantic_fixture.py`, generated by the current Editor's genuine
conversion test from public c0102 Git blobs. The decoder evidence retains the
original `e9fb4383...` seal (352 present sectors); current derivation and v5
package metadata have new hashes. All four documents are sealed together,
with full inventory/source checks and the same independent collision/population
oracles. The original tuple remains accepted. No runtime profile, map loader,
production trust rule or caller-supplied bypass is changed by this test-fixture
refresh. The complete semantic suite must pass against the new tuple before
claiming its release acceptance.
